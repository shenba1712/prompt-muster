...
describe('usePromptManager', () => {
    describe('addPrompt', () => {
        ...
        it('adds new prompts to the front of the array', () => {
            const { result } = renderHook(() => usePromptManager());
            const input1: CreatePromptInput = { title: 'First', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
            const input2: CreatePromptInput = { title: 'Second', content: 'b', model: 'gpt-4o', category: 'testing', tags: [] };
    ...
    describe('updatePrompt', () => {
        it('is a no-op when updating non-existent prompt', () => {
            const { result } = renderHook(() => usePromptManager());
            const input: CreatePromptInput = { title: 'A', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
        ...
        it('changes only the updated fields', () => {
            ...
            const originalUpdatedAtTime = result.current.prompts[0].updatedAt.getTime();
            ...
            expect(updated.updatedAt.getTime()).toBeGreaterThanOrEqual(originalUpdatedAtTime);
        });
    ...
    describe('deletePrompt', () => {
        it('removes exactly one prompt by id', () => {
            ...
            const input1: CreatePromptInput = { title: 'First', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
            const input2: CreatePromptInput = { title: 'Second', content: 'b', model: 'gpt-4o', category: 'testing', tags: [] };
            const input3: CreatePromptInput = { title: 'Third', content: 'c', model: 'gpt-4o', category: 'testing', tags: [] };
        ...
        it('decrements favoriteCount when deleting a favorite', () => {
            const { result } = renderHook(() => usePromptManager());
            const input: CreatePromptInput = { title: 'Fav', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
    ...
    describe('toggleFavorite', () => {
        it('toggles isFavorite and updates favoriteCount', () => {
            const { result } = renderHook(() => usePromptManager());
            const input: CreatePromptInput = { title: 'T', content: 'c', model: 'gpt-4o', category: 'testing', tags: [] };
    ...
    describe('filtering', () => {
        it('narrows filteredPrompts when model filter is set', () => {
            const { result } = renderHook(() => usePromptManager());
            const input1: CreatePromptInput = { title: 'A', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
            const input2: CreatePromptInput = { title: 'B', content: 'b', model: 'claude-sonnet', category: 'testing', tags: [] };
        ...
        it('narrows filteredPrompts when search filter is set', () => {
            const { result } = renderHook(() => usePromptManager());
            const input1: CreatePromptInput = { title: 'Code Review', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
            const input2: CreatePromptInput = { title: 'Unit Test', content: 'b', model: 'gpt-4o', category: 'testing', tags: [] };
        ...
        it('clears filters and shows all prompts again', () => {
            const { result } = renderHook(() => usePromptManager());
            const input1: CreatePromptInput = { title: 'A', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] };
            const input2: CreatePromptInput = { title: 'B', content: 'b', model: 'claude-sonnet', category: 'debugging', tags: [] };
            ...
            act(() => {
                result.current.setFilter({ model: 'all', category: 'all', search: '', showFavorites: false });
            });
    ...
    describe('filtering', () => {
        it('partial filter updates preserve existing filter state', () => {
            ...
            act(() => {
                result.current.addPrompt({ title: 'A', content: 'a', model: 'gpt-4o', category: 'testing', tags: [] });
                result.current.addPrompt({ title: 'B', content: 'b', model: 'claude-sonnet', category: 'debugging', tags: [] });
            });
        ...
        it('combines multiple filters with AND semantics', () => {
            ...
            act(() => {
                result.current.toggleFavorite(result.current.prompts.find(p => p.title === 'Code Review')!.id);
            });
