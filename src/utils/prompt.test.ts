...
describe('createPrompt', () => {
    it('generates a unique id', () => {
        const a = createPrompt({title: 'A', content: 'x', model: 'gpt-4o', category: 'testing', tags: []});
        const b = createPrompt({title: 'B', content: 'y', model: 'gpt-4o', category: 'testing', tags: []});
        expect(a.id).not.toBe(b.id);
    });

    it('defaults isFavorite to false', () => {
        const prompt = createPrompt({title: 'A', content: 'x', model: 'gpt-4o', category: 'testing', tags: []});
    ...
    it('preserves all input fields', () => {
        const input = {title: 'A', content: 'x', model: 'gpt-4o' as const, category: 'testing' as const, tags: ['t1']};
...
describe('isCategory', () => {
    it.each(CATEGORY_OPTIONS)('returns true for valid category %s', (category) => {
