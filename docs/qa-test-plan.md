...
| **Created** | 2026-07-15 |
| **Method** | Every row in [compliance-matrix.md](compliance-matrix.md)'s 43 requirements (33 functional + 10 non-functional, recounted 2026-08-04 — the prior "40 (30+10)" figure was a miscount that had propagated here uncorrected, audit finding #21) gets a test-case reference here — this is that matrix's `Verify` column, made concrete. |

---

## 0. Scope & method

[compliance-matrix.md](compliance-matrix.md) proved every requirement has a *design* home.
...
real test cases, not just re-stating verification *methods*.
...
| **E2E** | Playwright *(recommended — see [§6](#6-open-questions))* | Full user journeys, [ux-flows.md](ux-flows.md) | No — mocked provider responses |
...
already committed the CI pipeline to this. This document extends that rule to *all*
...
| FR-LIB-04 | Search/filter over file-backed library | Design gap closed 2026-08-04 (tickets 08.5/09.2/09.5 — compliance-matrix.md v0.7); no longer blocked-pending-design, but no `TC-*` case is written yet — allocate one when 09.5 lands |
...
| ID | Requirement | Test coverage |
|---|---|---|
| FR-COST-01 | Log every execution | TC-COST-01 (every `execute()` call — success *or* failure — writes a row) |
...
| NFR-04 | SQLite/Postgres via one interface | **Blocked-pending-implementation** (added 2026-08-04, finding #22) — TC-NFR-02 (identical suite passes against both engines — the concrete test of [ADR-003](adr/ADR-003-sqlite-local-postgres-team.md)'s claim) cannot execute until the Postgres implementation exists (Phase 4, #36); listed here as a planned case, not a currently-runnable one — unlike FR-LIB-04/FR-COST-02 this was previously listed with no such caveat, implying false present-day coverage |
...
| ID | Case | Expected result |
|---|---|---|
| TC-EVAL-01 | Run the same (prompt content + variables + model + params) twice | Second call is a cache **hit** — no new provider call; the new `eval_result` points at the *existing* `execution_run` ([database-schema.md §2.5](database-schema.md)) |
...
|---|---|---|---|
| TC-SEC-001 | P1 | Grep full log output (success *and* error paths) for the configured API key substring | Zero matches, always |
...
2. **The five undetailed E2E flows** ([§3.5](#35-e2e-ui-flow-tests)) — TC-E2E-003 through
   ...
   bulk.
3. **FR-COST-02 stays untestable until its design gap closes** — this document can't
   manufacture coverage for a screen that hasn't been designed. Tracked, not solved, here.
   (FR-LIB-04's design gap closed 2026-08-04 — see its row above; only its `TC-*` case
   allocation remains outstanding, not a design blocker.)

---

## Changelog

- **v0.3 (2026-08-04)** — Multi-angle audit fixes: corrected the requirement-count header
  (43 total, 33 functional — not 40/30, finding #21); FR-LIB-04 unblocked to match
  compliance-matrix.md v0.7 (design gap closed, only a `TC-*` allocation remains, finding
  #26); TC-NFR-02 (SQLite/Postgres parity) explicitly flagged blocked-pending-implementation
  since Postgres doesn't exist until Phase 4 — it was listed as a plain executable case with
  no such caveat, unlike its peers (finding #22). Also: TC-SEC-*/TC-E2E release-blocking
  cases (§5) should be cited as acceptance criteria on the tickets that build the
  corresponding behavior (chunks #17, #20, #23, #16, #28 in tickets.md) — none currently
  reference a TC-ID (finding #9); add those references as each ticket is picked up.
