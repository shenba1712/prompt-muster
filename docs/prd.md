...
community feed — it is _your own, tested, versioned prompt library_, portable across the
...
stakes — the LLM vendors' own consoles do it. The _live_ 2026 problem is **"I changed my
...
| **3** | **Prosumer / no-code operators**                                             | Hosted, standalone. A _different business_ — only if Ring 2 wins.                                                                         | Future / maybe |

**On "why leave non-developers out":** we don't — we sequence them. The differentiation
is _where the product lives_ (files, git, IDE, CLI). Non-devs have none of those, so
...
anyway. That dashboard is Ring 2's on-ramp _and_ the seam where monetization lives.
...
- **Priya, the daily driver (Ring 0/1).** Ships features with AI assistance. Wants her
  ...
  prompt tweak didn't quietly regress. _Success:_ she reaches for PromptMuster without
  thinking.
...
- **Messages** — a role-tagged array (system / user / assistant), _not_ a single content
...
### 6.1 Pillar 1 — MCP server _(the distribution channel and the flywheel)_

Expose the prompt library as MCP tools so prompts are retrievable and runnable from Claude
Code, Cursor, and Claude Desktop. Example: _"Run my `thorough-code-review` prompt on this
file."_ The IDE agent fetches the prompt, fills context, executes.
...
_Note:_ IDE agents already ship a primitive version of "prompt files in a repo" (Claude
...
### 6.2 Pillar 2 — Eval runner _(the reason people consider the tool)_
...
### 6.3 Pillar 3 — Web dashboard _(the human surface, and Ring 2's door)_
...
- **Multi-provider execution** with streaming (OpenAI, Anthropic, Google) behind a
  hand-built adapter.
- **Cost preflight + tracking** — token count and price _before_ running (labeled estimate
...
- **CI GitHub Action** — run prompt evals on every PR and comment results. (This replaces
  the old auto-PR-_reviewer_ webhook idea, which was a separate crowded product; the CI
...
- **P2** Export/import as JSON/YAML with schema versioning _(mostly free via files)_.

### 7.6 Sharing / team _(Ring 2)_
...
- **Persistence:** SQLite for local runs/logs (a "runs on your machine" tool cannot require
  ...
  backend. _(This is the deliberate resolution of the "learn Postgres" goal vs. the
  local-first product need — Postgres moves to the team tier.)_
...
| ---------------------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---- |
| **0 — Prototype** _(done / in progress)_ | Local prompt manager    | CRUD, filter, search, tests, shadcn UI                                                                                  | 0    |
...
- _Leading indicator of failure:_ if I'm not reaching for it by ~Phase 2, stop and ask why
...
layer) is kept _possible_ by the architecture, not pursued now.
...
| **Employer IP claim on a side project**                                                      | High — could void ownership                                           | Resolve _before_ first public commit (see below)                                                                                                                                                                                                  |
...
1. **Product name — ✅ DECIDED 2026-07-16: PromptMuster** (npm free, GitHub org free,
   checked live). The double meaning is the product: a _muster_ is a roll-call/assembly
   ...
   undersells _testing_, chronically misspelled); PromptCI (free; too narrow — one
   ...
   the Gauntlet AI bootcamp brand). Dismissed early: PromptVault (signals _storage_ —
...
2. **File format — ✅ DECIDED 2026-07-16: dotprompt (`.prompt`)**, options pitched against
   ...
   core libraries in JS/TS/Python/Go/Rust/Java, VS Code _and_ JetBrains plugins, a
...
- **Backlog re-cut** — [../core/backlog.md](core/backlog.md), re-tiered around these
  ...
  working. _(Done 2026-07-16 — the DIRECTION CHANGE banner is gone.)_
- **Ticket board** — [../core/tickets.md](core/tickets.md): every backlog feature cut
  into 1–5 SP tickets with a dependency spine _(done 2026-07-16)._
- **Technical design / architecture doc** — [trd.md](trd.md): file format spec, provider
  adapter, eval engine, MCP server, persistence.
- **ADRs** — _written:_ [../adr/](adr/README.md), ADR-001…ADR-008, all Accepted.
- **Reconciliation** — _2026-07-15:_ [readme.md](../README.md) rewritten,
  ...
  scheduled-change notes. _2026-07-16:_ backlog re-cut landed; the PromptLab→PromptMuster
...
- **v0.6 (2026-07-16)** — Product name decided: **PromptMuster** (npm + GitHub org free;
  full passed-over list recorded in §11.2 #1). No open decisions remain. _(Superseded by
  v0.7 on the rename mechanics: it lands in place on the existing `promptlab` repo, not at
  a future repo-creation event.)_
