...
5. **Does Library need a "Home" layer above it?** ✅ **Resolved 2026-08-04 by owner,
   after a dedicated design session (see completion-log.md).** Library remains the
   default route and Home — no separate dashboard screen — but gets a summary strip
   (prompt count, runs today, cost this week) once #11/#14/#30's data actually exists.
   Considered and rejected: a full dedicated Home dashboard (recent runs, favorites,
   cost, quick-create) — real value once Evals also exists post-Phase 2, but building
   it against mostly-empty Phase-0 data would be decorating an empty room. Revisit after
   Phase 2 if the summary strip still feels insufficient. Tracked as backlog #11a.

6. **Is a bare read-only demo enough for a logged-out visitor (Pipeline C)?** ✅
   **Resolved 2026-08-04 — see devops-cicd.md §3.4.** No — a visitor arriving at the
   demo link directly (not via the README) needs framing on the page itself. Tracked as
   backlog #32a.

---

## Changelog
- **v0.4 (2026-08-04)** — §8 Q5/Q6 added and resolved: Library summary strip (not a full
  Home dashboard) deferred until Runs/Cost data exists (#11a); demo site gets its own
  landing content rather than relying solely on the README (#32a, devops-cicd.md §3.4).
