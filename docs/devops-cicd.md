...
### 3.4 The demo needs its own landing content, not just a README (added 2026-08-04)

Original plan (§3.1) assumed a visitor arrives via the README, so the demo itself could be
just the app in read-only mode. That assumption breaks the moment the demo link is shared
directly — a Slack drop, a Show HN comment, a LinkedIn post — with no README in between.
Someone landing cold on a bare sample-data dashboard has no idea what they're looking at or
why it's different from any other prompt-saving tool.

**Resolved:** the demo deployment's root route is a real landing page, not a bare drop into
Library — same deployment, same design-system.md tokens, no separate site/hosting
technology:

- Hero + one-line pitch
- Real product screenshots (gated on the visual-polish pass landing first — see backlog
  #07.6 — screenshotting the current gradient-header/stock-shadcn state would undercut the
  pitch it's trying to make)
- The differentiation narrative already committed to in [core/backlog.md #32](core/backlog.md)'s
  POSITIONING note ("local-first + git-native files + MCP-in-IDE" vs. promptfoo/PromptLayer)
  — written once, used on both the landing route and the README, not duplicated separately
- A "Try it live" CTA scrolling/linking into the existing read-only Library/Run screens
  from §3.1 — the landing content and the functional demo are one page, not two

This doesn't replace the README's own pitch (#32) — READMEs are still how people discover
the repo itself (search, GitHub browsing). It closes the specific gap where the *demo link*
is the first thing someone sees, with nothing to explain it. Tracked as backlog #32a.
