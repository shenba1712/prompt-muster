...
- **New (Week 4 close-out):** the product repo's own `docs/` mirror got ahead of this roadmap repo this session — real doc updates (backlog/tickets/dashboard/CLAUDE.md corrections, plus a from-scratch `docs/core/completion-log.md`) happened directly in `prompt-muster` between 2026-08-04 and 2026-08-10, reconciled back here. **Resolved 2026-08-25:** the roadmap repo is the source; `prompt-muster/docs/` is generated output. `scripts/sync-docs-to-product.sh` now refuses to run when a mirror file holds edits it did not write (SHA provenance in `scripts/.sync-manifest`), so a reversed direction aborts loudly instead of being silently overwritten — which had already cost `08.3`'s completion record, backported the same day. Roadmap-private files (`horizons.md`, `dsa-bank.md`, `bonus-bank.md`) are no longer mirrored at all, since the product repo is public and this one is not.
