...
| [x] 08.3 | 3 | Parser: `.prompt` → domain object (frontmatter + role-tagged body), refuse unknown major schemaVersion, error messages worth reading | 08.2 | YAML/frontmatter parsing, schema versioning as a contract — **DONE 2026-08-10**: `core/parse-prompt-file.ts`, `gray-matter`+`yaml` installed per trd.md's stack. Body-splitting verified against google/dotprompt's real `toMessages` source, not guessed. Picoschema (`input`/`output`) stays an uncompiled raw passthrough, deferred to 09.x as planned. A missing `ext.promptmuster` block rejects with `MISSING_REQUIRED_FIELD` — adopting a bare/foreign dotprompt file isn't supported yet, revisit only with a real driving use case. `vitest.config.ts`'s `include` widened to also run `core/**/*.test.ts`, since it previously only matched `src/`. |
