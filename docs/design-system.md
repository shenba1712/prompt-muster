...
| **Table row** (Eval Run results / Comparison columns) | pass, fail, **errored** (added v0.4) | default, hover | Row-level status stripe (functional, not decorative — see artifact-design's "severity stripe" guidance) + icon, matching Badge — status. `errored` (timeout/rate-limit/invalid-key on that one cell) is a distinct third state, not forced into pass/fail — a 3-provider matrix or comparison will regularly have one cell error while the rest succeed (tickets 20.4, 29.2) |
...
| **Error banner** (Run/Comparison, added v0.4) | rate-limited, invalid-key, timeout, dropped-mid-stream | shown, dismissed | Distinct from the generic form-error state in §3's Input row — a *provider-call* failure, not a validation failure. Ticket 12.6 (tickets.md); every ux-flows.md journey was happy-path-only until this was named |

### 3.1 Interaction feedback policy (added v0.4, 2026-08-04)

Only two toasts are specified anywhere in this doc set today (save/commit, rollback —
[ux-flows.md](ux-flows.md) Flows 1, 5). Everything else has been left to be decided ad hoc,
which is how a product ends up with inconsistent feedback. The rule, applied going forward
(ticket 07.8):

- **Toast** — an action that succeeds off-screen or whose effect isn't otherwise visible:
  save/commit, rollback, "saved as baseline," budget-cap-hit.
- **Inline** — an action whose result IS the visible change: form validation errors (§3
  Input row), a run's output rendering, an eval matrix cell resolving.
- **Silent** — an action whose own visible state change is sufficient confirmation: toggling
  a favorite, a filter changing the list. Adding a toast here would be noise.

Motion tokens ([§2.6](#26-motion)) are specified but were not, on their own, wired into any
actual transition — the same "tokens on paper, unapplied in practice" gap this section's
toast policy closes. Ticket 07.8 applies both in the same pass.
...
## Changelog
- **v0.4 (2026-08-04)** — Findings from a dedicated "does this feel production-grade"
  design-review session (see completion-log.md). §3: Table row gains a distinct `errored`
  state (was pass/fail-only); new Error banner component for provider-call failures
  (rate-limit/invalid-key/timeout/dropped-stream) — every ux-flows.md journey was
  happy-path-only until now. New §3.1: interaction feedback policy (toast/inline/silent),
  closing the gap where only 2 toasts existed anywhere in the doc set; also flags motion
  tokens (§2.6) as defined-but-never-applied. Backing tickets: 07.8, 12.6, 13.3, 20.4, 29.2
  (tickets.md).
